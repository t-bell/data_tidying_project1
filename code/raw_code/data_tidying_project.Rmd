...
In this code chunk I am storing/saving the data objects under different file names and also saving them in a different folder location. The objects moved from folder '/code/raw_code/' to 'data/raw_data'.
...
df_department <- df1 %>% 
  ...
  mutate(`value` = gsub("[…]|[.]","",`Value (in millions)`)) %>%
  ...
  n <- summarize(mutate(df1, sum('value'))) %>% 
  ### create a new column called `business`
  ## set the value of this column to  be "department stores" 
  ## for the entire data set 
  mutate(business = gsub("[…]|[.]","",`Kind of business`)) %>%
  select(grepl('value, total |Department stores', business))       
  ## reorder column names
  df1[,order(colnames(df1),decreasing=TRUE)]
```

This code chunk has added the new columns: value and business. Unfortunately I still need help so the plots may or may not appear once knitting this document. :( sad face lol.
